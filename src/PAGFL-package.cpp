...
arma::mat getW(const arma::mat &X_block, const arma::mat &Z_block, const arma::vec &y, const unsigned int q)
{
  // Initial estimate with an identity weight matrix
  arma::mat Xt = X_block.t() * Z_block * arma::eye<arma::mat>(Z_block.n_cols, Z_block.n_cols) * Z_block.t();
  arma::mat XtX = Xt * X_block;
  arma::mat Xty = Xt * y;
  arma::vec beta = arma::inv(XtX) * Xty;
  // Obtain the var-cov matrix of the residuals
  arma::vec u = y - X_block * beta;
  arma::mat V = Z_block.t() * u * u.t() * Z_block;
  // Use the inverse as the subsequent weight matrix
  arma::mat W = invertV(V, q);
  return W;
}

// Computes the post-lasso estimates based on an estimated grouping
arma::mat getAlpha(const arma::mat &X, const arma::vec &y, const arma::mat &Z, const std::string &method, const unsigned int &n_periods, const unsigned int &N, const unsigned int &p, const arma::uvec &groups_hat)
{
  ...
  arma::vec alpha = arma::inv(groupXt * groupX) * groupXt * y_tilde;
...
arma::mat spjCorrec(const arma::mat &alpha_mat, arma::mat &X, arma::vec &y, const arma::mat &Z, unsigned int &n_periods, const unsigned int &N, const unsigned int &p, const arma::uvec &groups_hat, const std::string &method)
{
  ...
  // Select the way to net out fixed effects based on the estimation method
  ...
  arma::mat X_half, alpha_half, X_half_tilde;
  ...
  for (unsigned int i = 1; i < 3; i++)
  {
    ...
    X_half_tilde = netFEMat(X_half, n_periods / 2.0, N);
    y_half = splitVecInHalf(y, n_periods, N, i);
    y_half_tilde = netFEVec(y_half, n_periods / 2.0, N);
    // Estimate
    alpha_half = getAlpha(X_half_tilde, y_half_tilde, Z, method, n_periods / 2.0, N, p, groups_hat);
    // Store
...
arma::uvec getGroups(const arma::vec &beta, const arma::vec &y, const arma::mat &X, const arma::mat &Lambda, const unsigned int &p, const unsigned int &N, const double &tol)
{
  ...
  for (unsigned int i = 0; i < n; i++)
  {
    deltaNorm[i] = std::pow(arma::norm(delta.row(i), "fro"), 2);
  }
  ...
  if (groupListSize > 1)
  {
    ...
    {
      for (unsigned int j = fmin(i + 1, groupListSize - 1); j < groupListSize; j++)
      {
...
arma::uvec mergeTrivialGroups(arma::uvec &groups_hat, const arma::vec &y, const arma::mat &X, const arma::mat &Z, const std::string &method, const float &min_group_frac, const unsigned int &N, const unsigned int &n_periods, const unsigned int &p)
{
...
Rcpp::List PAGFL_Algo(arma::vec &y, arma::mat &X, const arma::mat &X_block, unsigned int &n_periods, const std::string &method, const arma::mat &Z, const bool &bias_correc, const double &lambda, const double &kappa, const double &min_group_frac, const unsigned int &max_iter, const double &tol_convergence, const double &tol_group)
{
  ...
  // Alternating direction method of multipliers (ADMM) penalty
  double varrho = 1.0;
  ...
  if (method == "PGMM")
  {
    Z_block = buildDiagX(Z, n_periods, N, arma::regspace<arma::uvec>(1, N));
    Xt = Xt * Z_block * arma::eye<arma::mat>(q * N, q * N) * Z_block.t();
  }
  arma::mat XtX = Xt * X_block;
  arma::mat Xty = Xt * y;
  ...
  if (method == "PGMM")
  {
    // Obtain weight matrix first by inverting the initial residual var-cov matrix
    u = y - X_block * beta;
    V = Z_block.t() * u * u.t() * Z_block;
    W = arma::inv(V);
    // W = arma::eye<arma::mat>(q * N, q * N);
  ...
  arma::uvec groups_hat_prelim = getGroups(beta, y, X, Lambda, p, N, tol_group);
  // Kick out trivial groups
  arma::uvec groups_hat;
  if (min_group_frac > 0.0)
  {
    groups_hat = mergeTrivialGroups(groups_hat_prelim, y, X, Z, method, min_group_frac, N, n_periods, p);
  }
  else
  {
  ...
  arma::mat alpha_mat = getAlpha(X, y, Z, method, n_periods, N, p, groups_hat);
  // Apply Split-panel Jackknife bias correction
  if (bias_correc)
  {
    alpha_mat = spjCorrec(alpha_mat, X, y, Z, n_periods, N, p, groups_hat, method);
  }
  // Reshape the beta vector into a N x p matrix
  arma::mat beta_mat = arma::reshape(beta, p, N).t();
...
double IC(const Rcpp::List &estimOutput, const arma::vec &y, const arma::mat &X_block, const double &rho)
{
  ...
  double fitness = arma::mean(arma::pow(y - X_block * alpha, 2));
  // Construct the IC
