...
Always stay up-to-date with the development version (1.1.5) of `PAGFL` from [GitHub](https://github.com/):
...
We suggest iterating over a comprehensive range of candidate values to trace out the correct model. To specify a suitable grid, create a logarithmic sequence ranging from a penalty parameter that induces complete slope heterogeneity to complete slope homogeneity (i.e., $\widehat{K} = 1$). The resulting $\lambda$ grid vector can be passed in place of any specific value, and a BIC IC selects the best-fitting parameter.
...
Lets delete 30% of observations, add indicator variables, and run `fuse_time()` again.
...
Besides not estimating the group structure, `grouped_plm()` (`grouped_tv_plm()`) behave identically to `pagfl()` (`fuse_time()`)
