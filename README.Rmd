...
In contrast to the time-constant `pagfl`, `dyn_pagfl` does not return a post-Lasso coefficient matrix but a $T \times p \times \widehat{K}$ array.
...
Lets delete a couple of observations, add indicator variables, and run `dyn_pagfl` again.
...
Furthermore, `dyn_pagfl` lets you specify a lot more optionalities than shown here. For example, it is possible to adjust the degree of the spline functions, the number of interior knots in the spline system, or estimate a panel data model with a mix of time-varying and time-constant coefficients. See `?dyn_pagfl()` for details.
