...
#'
#' @param y a \eqn{NT \times 1} vector or data.frame of the dependent variable, with \eqn{y=(y_1, \dots, y_N)^\prime}, \eqn{y_i = (y_{i1}, \dots, y_{iT})^\prime} and the scalar \eqn{y_{it}}.
#' @param X a \eqn{NT \times p} matrix or data.frame of explanatory variables, with \eqn{X=(x_1, \dots, x_N)^\prime}, \eqn{x_i = (x_{i1}, \dots, x_{iT})^\prime} and the \eqn{p \times 1} vector \eqn{x_{it}}.
...
#' } Default is \code{'PLS'}.
#' @param Z a \eqn{NT \times q} matrix of exogenous instruments, where \eqn{q \geq p}, \eqn{Z=(z_1, \dots, z_N)^\prime}, \eqn{z_i = (z_{i1}, \dots, z_{iT})^\prime} and \eqn{z_{it}} is a \eqn{q \times 1} vector. \eqn{Z} is only required when \code{method = 'PGMM'} is selected. When using \code{'PLS'}, either pass \code{NULL} or any matrix \eqn{Z} is disregarded. Default is \code{NULL}.
...
#' @param bias_correc logical. If \code{TRUE}, a Split-panel Jackknife bias correction is applied to the slope parameters (Dhaene & Jochmans, 2015). We recommend using this correction when facing a dynamic panel. Default is \code{FALSE}.
...
#' @param tol_group the tolerance limit for within-group differences. Two individuals are placed in the same group if the Frobenius norm of their coefficient parameter difference is below this parameter. If \code{NULL}, the heuristic \eqn{\sqrt{\frac{p}{\sqrt{NT} \log(\log(NT))}}} is used. We recommend the default \code{NULL}.
#' @param rho the tuning parameter balancing the fitness and penalty terms in the information criterion that determines the penalty parameter \eqn{\lambda}. If \code{NULL}, the heuristic \eqn{\rho = 0.07 \frac{\sqrt{NT} \log(NT)}{NT}} of Mehrabani (2023, sec. 6) is used. We recommend the default \code{NULL}.
#' @param varrho the non-negative Lagrangian \emph{ADMM} penalty parameter. For \emph{PLS}, the \eqn{\varrho} value is trivial. However, for \emph{PGMM}, small values lead to slow convergence of the algorithm. If \code{NULL}, the heuristic \eqn{\varrho = \max(\frac{\sqrt{5NTp}}{\log(NTp)}-7, 1}) is used. The default is \code{NULL}.
...
#'
#' Two individuals are allocated to the same group if \eqn{\| \hat{\beta}_i - \hat{\beta}_j \| \leq \epsilon_{\text{tol}}}, where \eqn{\epsilon_{\text{tol}}} is reflected by \code{tol_group}.
#'
#' We suggest identifying a suitable \eqn{\lambda} parameter by searching over a logarithmically spaced grid of candidate values with a lower limit of 0 and an upper limit that leads to a fully homogenous panel. A BIC-type information criterion then selects the best fitting \eqn{\lambda} value.
#'
#' @examples
...
PAGFL <- function(y, X, n_periods, lambda, method = 'PLS', Z = NULL, min_group_frac = .05, bias_correc = FALSE, kappa = 2, max_iter = 2e3, tol_convergence = 1e-3,
                  tol_group = NULL, rho = NULL, varrho = NULL, verbose = TRUE) {
  y <- as.matrix(y)
  X <- as.matrix(X)
  ...
  # If unspecified, set the tuning parameter of the information criterion as suggested in Mehrabani (2023, sec. 5)
  if (is.null(rho)) {
    rho <- .07 * log(N * n_periods) / sqrt(N * n_periods)
  }
  if (is.null(varrho)) varrho <- max(sqrt(5 * N * n_periods * p) / log(N * n_periods * p) - 7, 1)
