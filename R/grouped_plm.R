...
#'
#' @description Estimate a panel data model subject to an observed group structure. Slope parameters are homogeneous within groups but heterogeneous across groups.
...
#' where \eqn{\tilde{a}_{it} = a_{it} - T^{-1} \sum_{t=1}^T a_{it}}, \eqn{a = \{y, \bold{x}\}} to concentrate out the individual fixed effects \eqn{\gamma_i^0} (within-transformation).
#'
...
#' }
#' where \eqn{\bold{W}_k} is a \eqn{q \times q} p.d. symmetric weight matrix and \eqn{\Delta} denotes the first difference operator \eqn{\Delta \bold{x}_{it} = \bold{x}_{it} - \bold{x}_{it-1}} (first-difference transformation).
#'
#' @examples
